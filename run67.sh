#!/bin/bash
# Run with MapLibre plugin against Qt 6.7 (working baseline)
# Adjust these paths to match your environment:
MAPLIBRE_PKG=${MAPLIBRE_PKG:-""}      # Path to MapLibre Native Qt install prefix (built for Qt 6.7)
...
if [ -z "$MAPLIBRE_PKG" ]; then
    ...
    echo "  e.g.: MAPLIBRE_PKG=/path/to/maplibre-native-qt/install ./run67.sh"
    exit 1
fi
...
cd "$SCRIPT_DIR"
exec ./build67/reproducemapbug "$@" 2>&1
